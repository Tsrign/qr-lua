...
//  Copyright © 2016年 Tsrign. All rights reserved.
//
...
int qr_create(QR *qr){
    ...
    if(qr->logosize > 0){
        ...
        if(qr->logobg != NULL && qr->logobglen > 0){ //打上logobg
            ...
            gdImageCopyResampled(im, logobgim, (qr->size - qr->logosize)/2, (qr->size - qr->logosize)/2, 0, 0, qr->logosize, qr->logosize, gdImageSX(logobgim), gdImageSY(logobgim));
        ...
        if(qr->logo != NULL && qr->logolen > 0){ //打上logo
            ...
            //处理logo 等比缩放 add by weiwei39
            int bx,by,sx,sy;
            sx = qr->logosize;
            bx = gdImageSX(logoim);
            by = gdImageSY(logoim);
            sy = sx*by/bx;
            gdImageCopyResampled(im, logoim, (qr->size - (sx - qr->logomargin))/2, (qr->size - (sy - qr->logomargin))/2, 0, 0, sx-qr->logomargin, sy-qr->logomargin, gdImageSX(logoim ), gdImageSY(logoim ));
...
int main(int argc, char *argv[]){
    ...
    FILE *logofp = fopen("/data0/tmp/logo_big.png", "r");
    ...
    FILE *logobgfp = fopen("/data0/tmp/logo_style.png", "r");
    ...
    QR *qr = qr_init(str, 400, 10, "#000000", "#ffffff", 'M', 72, 80, logo, size1, logobg, size2, 8);
    ...
    if(len > 0){
        ...
        FILE *fp = fopen("/data0/tmp/logo_small.png", "w");
        fwrite(buf, len,  1, fp);
        fclose(fp);
        free(buf);
        qr_cleanup(qr);
