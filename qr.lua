...
--QR* qr_init(char *str, unsigned int size, unsigned int margin, char *fgcolor, char *bgcolor, char level, unsigned int dpi,unsigned int logosize, void *logo, unsigned int logolen, void *logobg, unsigned int logobglen, unsigned int logomargin);
function _M.get(data, size, margin, fgcolor, bgcolor, level, dpi, logosize, logo, logobg, logomargin)
    local cdata     = ffi.new("char[?]", #data+1, data)
    local csize     = tonumber(size)
    local cmargin   = tonumber(margin)
    local cfgcolor  = ffi.new("char[?]", #fgcolor+1, fgcolor)
    local cbgcolor  = ffi.new("char[?]", #bgcolor+1, bgcolor)
    ...
    if logo == nil then
        ...
        clogo = ffi.new("char[?]", #logo+1, logo)
        clogolen = #logo
    end
    ...
    if logobg == nil then
        ...
        clogobg = ffi.new("char[?]", #logobg+1, logobg)
        clogobglen = #logobg
    end
